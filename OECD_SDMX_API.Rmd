---
title: "OECD API"
output: html_notebook
---





```{r}
library(tidyverse)
library(rsdmx)
library(countrycode)
library(plotly)

```

```{r}
exporter_emission_data_url<- "https://stats.oecd.org/restsdmx/sdmx.ashx/GetData/IO_GHG_2021/FD_CO2..WLD.?startTime=2018&endTime=2018"

exporter_emissions_df <- as.data.frame(readSDMX(exporter_emission_data_url),labels=TRUE)

write.csv(exporter_emissions_df,"EEITF_Viz/exporter_emissions_df.csv")




country_df <- emissions_df%>%
  select(COU)

sector_df <- emissions_df%>%
  select(IND)

write.csv(country_df,"EEITF_Viz/country_df.csv",col.names=FALSE, row.names=FALSE)
write.csv(sector_df,"EEITF_Viz/sector_df.csv",col.names=FALSE, row.names=FALSE)




```


```{r}
importer_emission_data_url<- "https://stats.oecd.org/restsdmx/sdmx.ashx/GetData/IO_GHG_2021/DFD_FCO2..WLD.?startTime=2018&endTime=2018"

importer_emissions_df <- as.data.frame(readSDMX(importer_emission_data_url),labels=TRUE)

write.csv(importer_emissions_df,"EEITF_Viz/importer_emissions_df.csv")




```


```{r}
flows_emission_data_url<- "https://stats.oecd.org/restsdmx/sdmx.ashx/GetData/IO_GHG_2021/FD_CO2...DTOTAL/all?startTime=2018&endTime=2018"

flows_emissions_df <- as.data.frame(readSDMX(flows_emission_data_url),labels=TRUE)

flows_emissions_df_m<-flows_emissions_df%>%
  select(COU,PAR,obsValue)%>%
  mutate(x_region=countrycode(COU,origin = "iso3c",destination = "region23"))%>%
  mutate(i_region=countrycode(PAR,origin = "iso3c",destination = "region23"))%>%
  select(-COU,-PAR)%>%
  group_by(i_region,x_region)%>%
  summarize(obsValue=sum(obsValue))%>%
  pivot_wider(names_from = i_region,values_from = obsValue)

write.csv(flows_emissions_df_m,"EEITF_Viz/flows_m.csv")

```


```{r}
library(chorddiag)

m<-as.matrix(flows_emissions_df_m)

chorddiag(m)

```

